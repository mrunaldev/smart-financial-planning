...
function generateAlerts(data) {
    ...
    const { budgetBalance, emergencyFund, idealEmergencyFund, activeGoals, insurancePolicies, 
            ...
            actualCCOutstanding, outflows, now } = data;
    ...
    if (actualCCOutstanding > 50000) {
        alerts.push({
            type: 'info',
            icon: '💳',
            message: `High credit card usage: ${fmtMoney(actualCCOutstanding)}`,
            action: 'monthlyBudget'
        });
    }
...
export function renderDashboard(appData, netWorthSummary = {}) {
    ...
    // Monthly commitments should ONLY include Liability and Expenditure, NOT Saving/Investment
    const monthlyCommitments = recurringOutflows
        .filter(item => {
            const purpose = (item.purpose || '').toLowerCase();
            return purpose === 'liability' || purpose === 'expenditure' || purpose === 'loan' || purpose === 'others' || purpose === '';
        })
        .reduce((total, item) => (
    ...
    const totalCreditCardUsage = actualCCOutstanding;
    ...
    const alerts = generateAlerts({
        ...
        idealHealthInsurance, idealTermInsurance, healthInsurance, termInsurance,
        actualCCOutstanding, outflows, now
    });
                ...
                <div class="dash-alerts-carousel-track">
                    ${alerts.map((alert, index) => `
                        <div class="dash-alert-card dash-alert-${alert.type} ${index === 0 ? 'active' : ''}" onclick="switchToTab('${alert.action}')">
                            <div class="dash-alert-icon">${alert.icon}</div>
    ...
    grid.innerHTML = `
        <article class="dash-card dash-card-primary">
            ...
            <div class="dash-stat-row"><span class="dash-stat-label">Variable expenditure</span><span class="dash-stat-value">${fmtMoney(variableExp)}</span></div>
            <div class="dash-card-note">${recurringOutflows.length} recurring commitment${recurringOutflows.length === 1 ? '' : 's'} managed in <a href="#" onclick="switchToTab('outflow'); return false;" style="color:#3b82f6;text-decoration:underline;cursor:pointer;">Fixed Outflow</a> · <a href="#" onclick="switchToTab('monthlyBudget'); return false;" style="color:#3b82f6;text-decoration:underline;cursor:pointer;">View Budget</a></div>
        </article>

        <article class="dash-card">
            <div class="dash-card-header">
                <span class="dash-card-title">🏦 Accounts & Net Worth</span>
        ...
        <article class="dash-card">
            <div class="dash-card-header">
                <span class="dash-card-title">🎯 Goals & Investment Planning</span>
            ...
            <div class="dash-card-note">${plannedGifts > 0 ? `${fmtMoney(plannedGifts)} planned for gifts` : 'No gift budget recorded'} · <a href="#" onclick="switchToTab('financialGoal'); return false;" style="color:#3b82f6;text-decoration:underline;cursor:pointer;">View Goals</a></div>
        </article>

        <article class="dash-card">
            <div class="dash-card-header">
                <span class="dash-card-title">🛡️ Preparedness & Budget</span>
            </div>
            ...
            <div class="dash-card-note">${insuranceCount} polic${insuranceCount === 1 ? 'y' : 'ies'} tracked · Variable expenses + credit card spending</div>
        </article>
        
        ${insights.length > 0 ? `
        <article class="dash-card dash-insights" style="grid-column: 1 / -1;">
            <div class="dash-card-header">
                <span class="dash-card-title">💡 Insights & Recommendations</span>
